 ...
 *
 * This file was automatically generated by ${pkgName} v.${pkgVersion}
 *
 */
...
export async function typescriptOfSchema(
    ...
    let tableDefs = [] as TableDefinition[];
    if (tables.length === 0) {
    ...
    if (options.skipTables?.length) {
        tableDefs = tableDefs.filter((t) => !options.skipTables?.includes(t.tableName))
    }

    if (options.skipPrefix?.length) {
        tableDefs = tableDefs.filter(
            (t) => !options.skipPrefix?.some((prefix) => t.tableName.startsWith(prefix))
        )
    }

    tables = tableDefs.map((t) => t.tableName);
    ...
    const interfacePromises = tableDefs.map((tableDef) =>
        typescriptOfTable(db, tableDef, optionsObject)
    )
